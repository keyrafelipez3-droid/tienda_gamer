...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        ...
        .user-chip{display:flex;align-items:center;gap:8px;background:rgba(0,255,136,0.06);border:1px solid rgba(0,255,136,0.15);border-radius:20px;padding:6px 14px;font-size:0.82rem;text-decoration:none;color:#fff;transition:all 0.2s;}
        .user-chip:hover{background:rgba(0,255,136,0.1);color:#00ff88;}
...
<div class="content">
    <div class="container">
        ...
        <div class="empty-state">
            ...
            <a href="productos.php" style="display:inline-block;margin-top:16px;background:#00ff88;color:#000;font-weight:700;border-radius:10px;padding:10px 24px;text-decoration:none;">Ver todos</a>
        </div>
        <?php else: ?>
        <div class="row g-4">
            <?php
            ...
            <div class="col-md-6 col-lg-4 col-xl-3">
                <div class="prod-card" style="cursor:pointer;" onclick="window.location='producto_detalle.php?id=<?= $p['id_producto'] ?>'">
                    <div class="prod-img">
                        <?php if($img_src): ?>
                            <img src="<?= $img_src ?>" alt="<?= htmlspecialchars($p['nombre']) ?>">
                        <?php else: ?>📦<?php endif; ?>
                        <span class="prod-badge badge-<?= $badges[$bi % 3] ?>"><?= ucfirst($badges[$bi % 3]) ?></span>
                        <form method="POST" style="position:absolute;top:12px;right:12px;" onclick="event.stopPropagation()">
